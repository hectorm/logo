...
main() {
	...
	while [ "${vectorIndex:?}" -le "${vectorTotal:?}" ]; do
		...
		marginTotal="$(jq -nr --argjson d "${vector:?}" '$d.margins|length-1')"
		...
		while [ "${backgroundIndex:?}" -le "${backgroundTotal:?}" ]; do
			...
			while [ "${heightIndex:?}" -le "${heightTotal:?}" ]; do
				...
				marginIndex='0'
				while [ "${marginIndex:?}" -le "${marginTotal:?}" ]; do
					margin="$(jq -nr --argjson d "${vector:?}" --arg i "${marginIndex:?}" '$d.margins[$i|tonumber]')"

					inkscapeOpts="--export-filename=${TARGET_DIR:?}/${name:?}-${background#'#'}-h${height:?}-(${margin:?}).png"
					inkscapeOpts="${inkscapeOpts:?} --export-height=${height:?}"
					inkscapeOpts="${inkscapeOpts:?} --export-area=${margin:?}"
